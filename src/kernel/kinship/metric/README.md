...
| `fingerprint.zig` | Parallel bytes→record pass: byte-balanced sharding over `kernel/math/parallel.zig`, fail-to-`empty` degradation; the live rung and the atlas freshness fold both build through here                         |
